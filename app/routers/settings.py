...
async def _probe_provider(
    ...
    """Run an in-process health check for *provider* and return its status."""
...
async def settings_page(
    ...
    return _get_templates(request).TemplateResponse(request, "settings.html", {"settings": fields})
...
async def test_provider(
    ...
    """Test a provider connection with supplied credentials — does not write to DB.

    Blank credential fields fall back to the currently stored (decrypted) value so
    the caller can test just the URL without re-submitting the existing API key.
    """
...
async def save_settings_api(
    ...
    """Persist provider settings to DB.

    As a backstop, validates any provider whose connection credentials changed
    before writing.  Blank secret fields keep the existing stored value.
    Remote health checks run before any write so no DB transaction is held
    ...
    def _changed(key: str, submitted: str | None, *, secret: bool = False) -> bool:
        ...
        if secret and not submitted:
            return False  # blank secret means keep existing
